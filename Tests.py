...
def test_sort():
    ...
    testobject.sort_database(
        {"IMDb_Rating": "ASC", "Runtime": "DESC"},
        ["Room", "Shazam", "Warrior", "Chinatown"],
    )
...
def test_comprasion():
    assert testobject.compare_movies("OSCARS", ["Coco", "Moonlight", "Gods"]) == {
        "Moonlight": 3
    }
    assert testobject.compare_movies(
        "MOST_NOMINATIONS", ["Gods", "La La Land", "Kac Wawa"]
    ) == {"La La Land": 254}
    assert testobject.compare_movies(
        "All_awards", ["Gods", "La La Land", "Kac Wawa"]
    ) == {"La La Land": 215}
    assert testobject.compare_movies("IMDb_Rating") == {
        "The Shawshank Redemption": "9.3"
    }
    assert testobject.compare_movies(
        "Box_office", ["The Dark Knight", "Gone with the Wind"]
    ) == {"The Dark Knight": "$533,316,061"}
    assert testobject.compare_movies(
        "Runtime", ["The Dark Knight", "Gone with the Wind"]
